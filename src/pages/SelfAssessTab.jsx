...
import { SCHOOL_SELF_ASSESSMENTS, SCHOOLS } from '../data/schoolData';
...
export default function SelfAssessTab({ schoolId }) {
  ...
  const baseData = SCHOOL_SELF_ASSESSMENTS[schoolId];

  const [data, setData] = useState(baseData || {
    whodrives: [
      { area: 'School Staff & Coordinators', rating: null, notes: '' },
      { area: 'Community Partners',          rating: null, notes: '' },
      { area: 'Youth & Families',            rating: null, notes: '' },
    ],
    enabling: [
      { area: 'Shared Vision',             rating: null, notes: '' },
      { area: 'Trusting Partnerships',     rating: null, notes: '' },
      { area: 'Actionable Data',           rating: null, notes: '' },
      { area: 'Inclusive Decision Making', rating: null, notes: '' },
    ],
    keypractices: [
      { area: 'Collaborative Leadership, Shared Power & Voice', rating: null, notes: '' },
      { area: 'Powerful Student & Family Engagement',          rating: null, notes: '' },
      { area: 'Rigorous, Community-Connected Instruction',     rating: null, notes: '' },
    ...
    <div>
      <div style={{ marginBottom: 16 }}>
        ...
        <div style={{ fontSize: 12, color: '#8b8885', marginTop: 2 }}>
          Strategy Implementation Rubric: Emerging → Maturing → Transforming
      ...
      {sections.map(section => (
        <Card key={section.key} title={section.title} style={{ marginBottom: 12 }}>
          {section.items.map((item, idx) => (
            <AssessRow key={item.area} item={item} canEdit={canEdit} onUpdate={(updated) => updateItem(section.key, idx, updated)} />
          ))}
        </Card>
      ))}
    </div>
