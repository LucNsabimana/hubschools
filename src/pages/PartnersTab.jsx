...
export default function PartnersTab({ schoolId }) {
  ...
  const basePartners = SCHOOL_PARTNERS[schoolId] || [];
  const [partners, setPartners] = useState(basePartners);
  const [filter, setFilter] = useState('all');
  const [showAdd, setShowAdd] = useState(false);
  const [newPartner, setNewPartner] = useState({ name: '', type: 'Community-Based', focus: '', practice: '', boy: 'Active', mid: 'Active', progress: '' });

  const isMather = schoolId === 'mather';
  const inkindTotal = isMather ? MATHER_DATA.roi.inkind.reduce((s, r) => s + r.value, 0) : 0;
  const grantTotal  = isMather ? MATHER_DATA.roi.grants.reduce((s, r) => s + (r.awarded || 0), 0) : 0;
  ...
  const handleAdd = () => {
    ...
    setShowAdd(false);
    setNewPartner({ name: '', type: 'Community-Based', focus: '', practice: '', boy: 'Active', mid: 'Active', progress: '' });
  };
  ...
  return (
    <div>
      <div style={{ marginBottom: 16, display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
        ...
        {canEdit && <Button variant="orange" small onClick={() => setShowAdd(true)}>+ Add Partner</Button>}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(110px,1fr))', gap: 10, marginBottom: 14 }}>
        ...
        {isMather && <MetricCard label="In-Kind Value"   value={`$${inkindTotal.toLocaleString()}`} accent="blue" />}
        {isMather && <MetricCard label="Grants Awarded"  value={`$${grantTotal.toLocaleString()}`}  accent="teal" />}
      </div>

      {showAdd && (
        <Card title="Add New Partner" style={{ marginBottom: 12 }}>
          <div className="grid-2">
            <Field label="Organization Name" required><input value={newPartner.name} onChange={e => setNewPartner(p => ({ ...p, name: e.target.value }))} placeholder="Organization name" /></Field>
            ...
            <Field label="Key Practice"><select value={newPartner.practice} onChange={e => setNewPartner(p => ({ ...p, practice: e.target.value }))}>{['Collaborative Leadership','Expanded, Enriched Learning Opportunities','Rigorous Community-Connected Instruction','Culture of Belonging','Integrated Systems of Support','Powerful Student & Family Engagement',''].map(v => <option key={v} value={v}>{v || '— Select —'}</option>)}</select></Field>
            ...
            <tbody>
              {filtered.map((p, i) => (
                <tr key={i} style={{ borderBottom: '0.5px solid var(--border)' }} onMouseEnter={e => e.currentTarget.style.background='rgba(255,255,255,0.02)'} onMouseLeave={e => e.currentTarget.style.background='transparent'}>
                  ...
                  <td style={{ padding: '7px 8px', color: 'var(--text-muted)', fontSize: 11 }}>{p.practice}</td>
              ...
              {filtered.length === 0 && (
                <tr><td colSpan={7} style={{ padding: '24px', textAlign: 'center', color: '#8b8885', fontSize: 12 }}>No partners in this category.</td></tr>
              )}
            </tbody>
          </table>
        </div>
